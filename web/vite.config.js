...
import { renderShell, parseShellAttrs } from './epiella/shell-render.js';
import { readdirSync } from 'node:fs';
import { resolve } from 'node:path';

const EPIELLA_SHELL = resolve(import.meta.dirname, 'epiella/shell-render.js');

// Redirect Gentelella runtime imports of ./shell-render.js to the EpiElla overlay.
// epiella/shell-render.js still imports the real upstream file by absolute path.
function gentelellaOverlayPlugin() {
  return {
    name: 'epiella-gentelella-overlay',
    resolveId(source, importer) {
      if (!importer) return null;
      const norm = importer.replace(/\\/g, '/');
      if (norm.includes('node_modules/gentelella') && (source === './shell-render.js' || source.endsWith('/v4/shell-render.js'))) {
        return EPIELLA_SHELL;
      }
      return null;
    }
  };
...
function discoverEntries() {
  ...
  for (const file of readdirSync(dir)) {
    ...
    if (!EPIELLA_PAGES.has(file)) continue;
...
export default defineConfig(({ command }) => ({
  ...
  publicDir: 'public',
  plugins: [gentelellaOverlayPlugin(), shellInjectionPlugin()],
  logLevel: 'info',
  clearScreen: false,
  build: {
    outDir: 'dist',
  ...
  resolve: {
    alias: [
      {
        find: /^gentelella\/v4\/(.+)$/,
        replacement: `${resolve(import.meta.dirname, 'node_modules/gentelella/src/v4')}/$1`
      },
      {
        find: /^gentelella\/scss\/(.+)$/,
        replacement: `${resolve(import.meta.dirname, 'node_modules/gentelella/src/scss')}/$1`
      }
    ]
  },
  css: {
